---
title: "Telling Stories With Data: Showing Disparities in a Continuous Risk Factor"
...
> I am using the graphing library `ggplot`. As I often do, I am going to show the code. The code can be useful, especially if you are trying to replicate some of these graphs.  <span style="background-color: #FFFF00">However, the discussion is intended to be more conceptual, and you do not need to understand the code to follow the discussion. Indeed, the discussion is intended to be applicable to a wide variety of data visualization tools, including those that do not rely on coding. </span>
...
myplot1 <- ggplot(data = mydata,
  ...
  scale_fill_viridis_d() + # discrete viridis fills
...
In the graphs below, *group* will again go on the x axis.  *Risk* will go on the y axis.  In drawing this information in various ways--employing various geometries--I am going use colors (the *Viridis* color palettes) for fill and color. 
...
myplot + stat_summary(fun.y = mean, geom = "bar") +
...
myplot + stat_summary(fun.y = mean, geom = "point", size = 10) +
  ylim(0,150) + 
  labs(title = "Mean Risk By Group")
...
Perhaps we connect the points with a line.
...
myplot + geom_point() # simple points

```

```{r, fig.cap="Jittered Points"}

myplot +  geom_jitter() # jittered points

```

```{r, fig.cap="Beeswarm"}

myplot + geom_beeswarm() # beeswarm geometry

```

Personally, I like the compact nature of the *beeswarm* geometry, but that may be a matter of personal preference.
...
myplot + geom_boxplot(fill = "white") # boxplot
...
myplot + geom_violin() # violin plot

```

# Thinking More About Color

Above, we added color to the graph simply to add visual interest. We may wish to manually tweak the colors, to emphasize the level of risk faced by a particular group or groups.

This of course requires slightly more complicated code to tweak the colors.
...
myplot + stat_summary(fun.y = mean, geom = "bar") +
  ...
  scale_fill_manual(breaks = c("Group A", 
                               "Group B", 
                               "Group C"),
...
We should also think about whether careful attention to manual manipulation of choice of color, and making our titles and labels more clear would help to tell our story.
