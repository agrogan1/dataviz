...
myplot1 <- ggplot(data = mydata,
  ...
  stat_summary(fun.y = mean, geom = "bar") +
  labs(title = "Risk by Group") + # labels
  scale_color_viridis_d(option = "cividis") + # discrete viridis colors
  scale_fill_viridis_d(option = "cividis") + # discrete viridis fills
...
We can add a big number as a data label and for emphasis.
...
mydata %>% 
  group_by(group) %>% # group by group
  # summarize by group
  summarise(mean_risk = round(mean(risk), 
                              digits = 1)) %>% 
  ggplot(aes(x = group,
             y = mean_risk,
             fill = group,
