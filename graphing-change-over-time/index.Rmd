---
title: 'Telling Stories With Data: Graphing Change Over Time'
...
[^slopegraph]:  In order to be clear and effective, a slope graph may often only show the outcome at the beginning point, and at the end point.  A slope graph may be less satisfactory when there are multiple timepoints. The small multiple idea works with a slopegraph as well.
...
```{r, eval = TRUE, fig.caption = "Long Data", fig.margin=TRUE, fig.height= 9}
...
Many data sets, but not all, are originally created in the *wide* format--as shown below--where every row of data is an *individual*, and an individual only has a *single row*.  Ideally, every row in *wide* data is uniquely identified by an individual *id* number.  
...
Graphics made with [ggplot2](http://ggplot2.org/)created by Hadley Wickham.
