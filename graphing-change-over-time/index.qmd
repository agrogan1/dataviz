...
Data can be *reshaped* from *wide* to *long* format, and *vice versa*. Two straightforward options are the `reshape` command as available in both [Stata](http://www.stata.com) and the `pivot_*()` commands available in [R](https://www.r-project.org/).
